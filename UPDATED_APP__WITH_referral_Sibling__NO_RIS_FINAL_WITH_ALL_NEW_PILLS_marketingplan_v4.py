...
def _render_marketing_marketing_plan(
    ...
    with c1:
        chosen_sources = st.multiselect(
            ...
            default=None,
            key="mkt_plan_sources"
        )
    with c2:
        rank_by = st.selectbox("Top by", ["Deals", "Enrollments"], index=0, key="mkt_plan_rankby")
    with c3:
        topN = st.number_input("Number of countries", min_value=1, max_value=100, value=10, step=1, key="mkt_plan_topn")
    with c4:
        show_chart = st.checkbox("Show chart", value=True, key="mkt_plan_chart")
    ...
    # Base aggregations
    deals_by_country = df.loc[deals_mask].groupby(_country).size().rename("Deals")
    enrl_by_country  = df.loc[enrl_mask].groupby(_country).size().rename("Enrollments")
