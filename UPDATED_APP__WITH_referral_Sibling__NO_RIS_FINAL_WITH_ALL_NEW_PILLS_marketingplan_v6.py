...
def _render_marketing_marketing_plan(
    ...
    mode = st.radio(
        ...
        index=0,
        horizontal=True,
        key="__marketing_plan__mode"
    )
    preset = st.selectbox(
        "Date preset",
        ["This Month","Last Month","Today","Yesterday","Custom Range"],
        index=0,
        key="__marketing_plan__preset"
    )
    today = date.today()
    if preset == "This Month":
        start_d = date(today.year, today.month, 1)
        ...
        with c1d:
            start_d = st.date_input("Start", value=date(today.year, today.month, 1), key="__marketing_plan__start")
        with c2d:
            end_d = st.date_input("End", value=today, key="__marketing_plan__end")
        if start_d > end_d:
            st.warning("Start date is after end date."); return
    ...
    with c1:
        chosen_sources = st.multiselect(
            ...
            default=None,
            key="__marketing_plan__sources"
        )
    with c2:
        rank_by = st.selectbox("Top by", ["Deals", "Enrollments"], index=0, key="__marketing_plan__rankby")
    with c3:
        topN = st.number_input("Number of countries", min_value=1, max_value=100, value=10, step=1, key="__marketing_plan__topn")
    with c4:
        show_chart = st.checkbox("Show chart", value=True, key="__marketing_plan__chart")
...
# --- Marketing / Marketing Plan (explicit master gate) ---
try:
    if 'master' in globals() and master == "Marketing" and 'view' in globals() and view == "Marketing Plan":
        _render_marketing_marketing_plan(
            df_f=df_f,
            create_col=create_col if 'create_col' in globals() else None,
            pay_col=pay_col if 'pay_col' in globals() else None,
            source_col=source_col if 'source_col' in globals() else None,
